...
const BeaconMock = {
  ...
  createBeacon: async beacon => {
    ...
    const newBeacon = {
      ...beacon,
      id: beaconsTable.length ? Math.max(...beaconsTable.map(b => b.id)) + 1 : 1,
    };
...
export const BeaconClient = {
  getBeacons: () => (!env.useMock ? BeaconMock.getBeacons() : api.get('/beacons')),

  getBeaconById: id => (!env.useMock ? BeaconMock.getBeaconById(id) : api.get(`/beacons/${id}`)),

  createBeacon: data => (!env.useMock ? BeaconMock.createBeacon(data) : api.post('/beacons', data)),

  updateBeacon: (id, data) =>
    !env.useMock ? BeaconMock.updateBeacon(id, data) : api.put(`/beacons`, data),

  deleteBeacon: id => (!env.useMock ? BeaconMock.deleteBeacon(id) : api.delete(`/beacons/${id}`)),

  getBeaconTypes: () => BeaconMock.getBeaconTypes(),

  getBeaconByDistrictId: districtId =>
    !env.useMock
      ? BeaconMock.getBeaconByDistrictId(districtId)
      : api.get(`/beacons/district/${districtId}`),
};
